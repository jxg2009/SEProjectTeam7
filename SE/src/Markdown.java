...
public class Markdown {
	...
	public static void main(String[] args) {
		...
		while (!cmdSuccess) {
			...
			String[] array;
			array = cmd.split(" "); // split the cmd lines by space and insert
									// into array
			...
			if (cmd1.equals("help")) { // when cmd is "help"
				...
				/*
				 * when cmd is "cvt" there are four cases 1.cvt inputfile
				 * (space) outputfile (space) option(-pl, -st, -sl ) 2.cvt
				 * inputfile (space) outputfile 3.cvt inputfile (space) option
				...
				if (Pattern.matches(pattern, element)) {
					...
					if (Pattern.matches(pattern, element)) {
						...
						// when the user didn't give the output file name =>
						// default naming
		...
		for(int i = 0; i < inputFileList.size(); i++){
			String inputFileName = inputFileList.elementAt(i); // iputFileList에서 file명을 가져옴
			String outputFileName = null; // 저장할 file의 이름
			FileRead fileRead = new FileRead(inputFileName); // doc1.md를 바꾼다고 가정
			stringVector = fileRead.parseWithLine(); // separate by line

			LineParser lineParser = new LineParser();
			stringVector = lineParser.parseLine(stringVector); // process with lines
																// to make html
																// element. 라인 단위 처리

			MdParser mdParser = new MdParser();
			stringVector = mdParser.parseWhiteSpace(stringVector); // 라인단위로 처리된 벡터를
																	// 단어 단위로 쪼개서
																	// Sring vector에
																	// 저장
	...
	System.out.println("++++" + stringVector.elementAt(j));
				resultString += stringVector.elementAt(j) + " "; // 한줄로 만들기
			}
			
			outputFileName = outputFileList.elementAt(i);
			
			try { // outpufileName으로 .html 파일에 저장
				BufferedWriter fw = new BufferedWriter(new FileWriter(outputFileName, false));
